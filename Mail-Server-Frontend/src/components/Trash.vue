    ...
    <v-card app height="655" width="200">
      ...
      <v-layout row>
        <v-btn block height="45px" v-for="(email,i) in emails" :key="i" v-on:click=openEmail(i) >
          {{email.Subject}}
          <v-btn class="success mx-10" @click="undoRemove(i)">undo delete</v-btn>
        </v-btn>
      </v-layout>
      </v-col>
    </v-row>
...
export default {
    ...
    methods: {
      undoRemove(index){
        ...
        axios.post("http://localhost:8080/undo_delete",formData)
        .then(() => {
          this.$router.push("/trash/"+this.$route.params.Email)
        })
      },
      forward(){
        this.page++
    ...
    data(){ 
      ...
      Path: "\\src\\main\\java\\demo\\blitz\\Data\\Users\\" + this.$route.params.Email + '\\Trash',
